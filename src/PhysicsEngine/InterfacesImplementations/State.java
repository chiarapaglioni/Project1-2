...
public class State implements StateInterface {
    ...
    public void initializeState() {
        ...
        for (int i = 0; i < state.length; i++) {
            ...
            if(DEBUG){
                System.out.println("PhysicsEngine.PhysicsEngine.Planets.InterfacesImplementations.State - initState position " + Planet.planets[i].name + " " + state[i][1]);
                System.out.println("PhysicsEngine.PhysicsEngine.Planets.InterfacesImplementations.State - initState velocity " + Planet.planets[i].name + " " + state[i][0]);
            }
        }
    }
    ...
    public StateInterface addMul(double step, RateInterface rate) {
        ...
        if(DEBUG){
            ...
            System.out.println("PhysicsEngine.PhysicsEngine.Planets.InterfacesImplementations.State - position after update " + Planet.planets[i].name + " " + newState.getPos(i));
            System.out.println(("PhysicsEngine.PhysicsEngine.Planets.InterfacesImplementations.State - velocity after update " + Planet.planets[i].name + " " + newState.getVel(i)));
        }
        return newState;
